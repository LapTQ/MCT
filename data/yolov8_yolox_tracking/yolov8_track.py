...
def run(
    ...
    t = (t[0] + t[1] + t[2], 1E3 / (t[0] + t[1] + t[2]), t[3], 1E3 / t[3], sum(t), 1E3 / sum(t))
    # LOGGER.info(f'Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS, %.1fms {tracking_method} update per image at shape {(1, 3, *imgsz)}' % t)
    LOGGER.info(
        f'Speed: %.1fms pre-process + inference + NMS (%.1f FPS), %.1fms {tracking_method} update (%.1f FPS), %.1fms total (%.1f FPS) per image at shape {(1, 3, *imgsz)}' % t)
