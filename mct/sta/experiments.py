...
from mct.utils.draw import plot_roi, plot_skeleton_kpts, plot_box, plot_loc
from mct.utils.general import map_mono
from mct.sta.base import ConfigPipeline
from mct.sta.engines import MyQueue, Scene, Tracker
from mct.sta.pipeline import CameraPipeline, ExportPipeline, SCTPipeline, STAPipeline, SyncPipeline
from mct.utils.load import load_homo, load_roi
...
def eval_reid(
    ...
    with open(pred_reid_path, 'r') as f:
        ...
        pred = [i for i in pred if 
            ((i[0] == f'CAM_{cam1_id}' or i[0] == f'CAM_{cam2_id}') if not using_PETS09 else
             (i[0] == f'CAM_00{cam1_id}' or i[0] == f'CAM_00{cam2_id}'))
        ]
...
def run(VIDEO_SET):
    ...
    TRACKER_SET = [
        ...
        'GTSplittedTrackerbox',
        'YOLOv7pose_pretrained-1920-ByteTrack',
    ]
    

    video_set = 'PETS09'
    tracker_name = 'GTSplittedTrackerbox'
    for config_pred_option in [0]:

        print(f'Running prediction config = {config_pred_option}')
        ...
        for video_id in range_:
            ...
            roi_path = str(Path(video_set_dir) / f"roi_{cam2_id}{'' if video_set != 'PETS09' else f'_wrt_{cam1_id}'}.txt")
            ...
            #     # can be out_validate_pred_mct_trackertracker_path if config 18th of pose does not exist
            #     # str(Path(video_set_dir) / 'YOLOv7pose_pretrained-640-ByteTrack-IDfixed' / 'pred' / '18_val' / f'{cam1_id}_{cam2_id}_{video_id}.txt')
            #     out_validate_pred_mct_trackertracker_path
            # ))
            ...
            eval_reid(
                ...
                video_id, 
                pred_reid_path, 
                *((out_validate_pred_mct_trackertracker_path, False) if video_set != 'PETS09' else (out_validate_pred_mct_trackertracker_path, True))
            )
