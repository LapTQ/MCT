...
from mct.utils.pipeline import MyQueue, Config, Camera as PLCamera, Tracker, SCT as PLSCT, Visualize as PLVisualize, Scene, STA as PLSTA, SyncFrame as PLSync
...
def async_startup(app):
    with app.app_context():
        ...
        for cid, cv in cams.items():
            ...
            pl_sct = PLSCT(config, tracker, iq_sct, [iq_vis_sct_annot], name=f'SCT<{cid}>')
        ...
        for r in Region.query.filter_by(type='overlap').all():
            ...
            stas[(csid, cpid)] = {
                'pl_sync': PLSync(config, [iq_sync_s, iq_sync_p], iq_sta_sync, name=f'STA<({csid}, {cpid})>'),
                'pl_sta': PLSTA(config, [ss, ps], H, [iq_sta_sct_s, iq_sta_sct_p], iq_sta_sync, name=f'Sync<({csid}, {cpid})>')
            }
